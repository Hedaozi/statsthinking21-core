...
Table \@ref(tab:PhysActiveTable)展示了不同数值的频数：`r I(PhysActive_table %>% subset(PhysActive=='Yes') %>% dplyr::select(AbsoluteFrequency))`人选择了“Yes”，`r I(PhysActive_table %>% subset(PhysActive=='No') %>% dplyr::select(AbsoluteFrequency))`人选择了“No”，而`r I(PhysActive_table %>% subset(is.na(PhysActive)) %>% dplyr::select(AbsoluteFrequency))`人没有回答。我们将这称为“频数分布”（*frequency distribution*）。频数分布给出了每个可能的取值在样本中出现的频数。
